...
#define Setup ios::sync_with_stdio(false);cin.tie(NULL);cout.tie(NULL);int t,n,x,y,z;string s;long long int sum=0,X,Y,Z;
...
int main() {
    ...
    cin>>s;
    X=1;
    for(int i=s.size()-1;i>0;i--) sum+=pow(2,i);
    for(int i=s.size()-1;i>=0;i--,X*=2) if(s[i]=='7') sum+=X;
    cout<<++sum<<endl;
}
